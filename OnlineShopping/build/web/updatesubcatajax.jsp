...
<%
        String cname="" ,catid="",oldpic="",imagename="", cid="",path="";
        ...
        if(isMultipart==true)
        {
           ...
           for(FileItem myitem:items)
           {
                if (myitem.isFormField())
                {
                    ...
                    if(itemName1.equals("subcatname")) //control's name - textbox name
                    {
                        cname=value;
                    }
                    else if(itemName1.equals("oldpicname")) //control's name - textbox name
                    {
                        oldpic=value;
                    }
                    else if(itemName1.equals("oldsubid")) //control's name - textbox name
                    {
                        cid=value;
                    }
               ...
               try
                {
                   ...
                   try
                   {
                        String q = "update addsubcat set maincatid=?,subcatname=?,subcatpic=? where subcatid=?";
                        PreparedStatement myst =  myconn.prepareStatement(q);
                        myst.setString(1, catid);
                        myst.setString(2, cname);
                        myst.setString(3, oldpic);
                        myst.setString(4, cid);
